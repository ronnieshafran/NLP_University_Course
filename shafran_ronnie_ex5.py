...
# an object to represent all the data needed for each cell in the chart
class ChartEntry:
    def __init__(self, probability: float = 0, rule: Rule = None, left_coords=None, right_coords=None):
        self.probability = probability
        self.rule = rule
        # binary tree properties thanks to Chomsky
        self.right = right_coords
        self.left = left_coords
...
# cky algorithm implementation as shown in the lectures
def cky(sentence: "", grammar: GrammarRules):
    words = parse_line(sentence)
    n = len(words)
    ...
    for col in range(1, n + 1):
        ...
        for A in grammar.get_possible_rules((word,)):
            ...
            chart[col - 1][col][A.index].text = word
            chart[col - 1][col][A.index].is_terminal = True
                    ...
                    for C in [rule for rule in grammar.rules if chart[k][col][rule.index].probability > 0]:
                        for rule in grammar.get_possible_rules((B.source, C.source)):
                            ...
                            if p > chart[row][col][rule.index].probability:
                                ...
                                chart[row][col][rule.index].left = chart[row][k][B.index]
                                chart[row][col][rule.index].right = chart[k][col][C.index]
    result_cell = chart[0][n][0]
    tree_builder = TreeBuilder()
    if result_cell.probability > 0:
        tree_builder.trace_parsing_tree(result_cell)
        return tree_builder.tree, log(result_cell.probability)
    return None, None
...
class TreeBuilder:
    ...
    # traverse the tree pre-order style
    def trace_parsing_tree(self, chart_entry: ChartEntry, indent=0):
        if chart_entry.is_terminal:
            self.tree += f'\n{indent * TAB} {chart_entry.rule.source} -> {chart_entry.text}'
            return
        self.tree += f"\n{indent * TAB} {chart_entry.rule.source} -> "
        self.trace_parsing_tree(chart_entry.left, indent + 1)
        self.trace_parsing_tree(chart_entry.right, indent + 1)
